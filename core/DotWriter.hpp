...
public:
    ...
    void writeFile(const char* filename, std::vector< std::pair <Tedge,Tedge> > *edges_pairs )
    {
        std::vector< std::string > *names = NULL;
        std::vector< float > *weights = NULL;
        writeDOT(filename, edges_pairs, names, weights);
    }
    
    template <typename Tedge, typename Tw >
    void writeFile(const char* filename, std::vector< std::pair <Tedge,Tedge> > *edges_pairs, std::vector< Tw > *weights  )
    {
        writeDOT(filename, edges_pairs, NULL, weights);
    }
    
    template <typename Tedge, typename Tw >
    void writeDOT(const char* filename, std::vector< std::pair <Tedge,Tedge> > *edges_pairs, std::vector< std::string > *names, 
