function [F] = fundamentalMatrix( xd1, xd2 )
...
leasqrfunc = @(x, p) costFundamental(x,p); 
%  leasqrdfdp = @(x, f, p, dp, func) [exp(-p(2)*x), -p(1)*x.*exp(-p(2)*x)];
Func_name = leasqrfunc;
dp = [0.001; 0.001];
stol=0.001; niter=50;
%  derivativeFundamental(xin,pin);

global verbose;
verbose = 0;
[f1, solut, cvg, iter1] = leasqr (xin, y, pin, Func_name , stol, niter);
%  if (cvg == 1) fprintf('Convergence of Fundamental Matrix with %i iterations\n',iter1); end;
Prcv=reshape(solut(1:12),3,4);
